...
void print_ROB_entry(int i, int valid, int T, int T_old, int dest_idx, int complete, int halt, int illegal){
  if (ppfile != NULL)
    fprintf(ppfile, "index: %3d | valid: %1d | T: %3d | T_old: %3d | dest_idx: %3d | complete: %1d | halt: %1d | illegal: %1d |\n", i,valid, T, T_old, dest_idx, complete, halt, illegal);
}
